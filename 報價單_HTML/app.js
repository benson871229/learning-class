...
(function () {
  ...
  function makeWord() {
    ...
    if (!name) { showMsg('請填寫學生姓名', 'err'); return; }
    var rows = readRows();
    if (!rows.length) { showMsg('請至少填寫一筆課程（含課程名稱）', 'err'); return; }
    ...
    try {
      var content = state.templateBuf
        ? new Uint8Array(state.templateBuf)
        : base64ToUint8(TEMPLATE_B5_BASE64);

      var doc = new window.docxtemplater(new PizZip(content), {
        paragraphLoop: true,
        linebreaks: true
      });
      doc.render({
      ...
      saveAs(blob, '報價單_' + safeFileName(name) + '.docx');
      showMsg('Word 已下載（B5 版面，可直接用 Word 編輯）');
    } catch (e) {
      var detail = e.properties && e.properties.errors && e.properties.errors.length
        ? e.properties.errors[0].properties.explanation : e.message;
      showMsg('產生 Word 失敗：' + detail, 'err');
    }
  }
